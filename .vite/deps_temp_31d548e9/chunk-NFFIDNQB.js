import { getOverlayAlpha_default } from "./chunk-FVZ2FESL.js";
import {
  ThemeProvider_default,
  createCssVarsProvider,
  createGetCssVar,
  prepareCssVars_default,
} from "./chunk-KUU34DBE.js";
import {
  _extends,
...
function shouldSkipGeneratingVar(keys) {
  ...
  return (
    ...
    (keys[0] === "palette" &&
      !!(
        (_keys$ = keys[1]) != null &&
        _keys$.match(/(mode|contrastThreshold|tonalOffset)/)
      ))
  );
}
...
function setColorChannel(obj, key) {
  if (!(`${key}Channel` in obj)) {
    obj[`${key}Channel`] = (0,
    import_colorManipulator.private_safeColorChannel)(
      toRgb(obj[key]),
...
function extendTheme(options = {}, ...args) {
  ...
  const {
      ...
      shouldSkipGeneratingVar:
        shouldSkipGeneratingVar2 = shouldSkipGeneratingVar,
    } = options,
    input = _objectWithoutPropertiesLoose(options, _excluded4);
  const getCssVar = createGetCssVar2(cssVarPrefix);
  const _createThemeWithoutVa = createTheme_default(
      _extends(
        {},
        input,
    ...
    muiTheme = _objectWithoutPropertiesLoose(
      _createThemeWithoutVa,
      _excluded22,
    );
  const { palette: darkPalette } = createTheme_default({
    palette: _extends(
      {
        mode: "dark",
      },
  ...
  Object.keys(theme.colorSchemes).forEach((key) => {
    ...
    if (key === "light") {
      ...
      const snackbarContentBackground = (0,
      import_colorManipulator.private_safeEmphasize)(
      ...
      setColor(
        ...
        "fabHoverBg",
        (0, import_colorManipulator.private_safeEmphasize)(
      ...
      setColor(
        ...
        "border",
        (0, import_colorManipulator.private_safeLighten)(
          (0, import_colorManipulator.private_safeAlpha)(palette.divider, 1),
          0.88,
        ),
      );
      setColor(
        palette.Tooltip,
        "bg",
        (0, import_colorManipulator.private_safeAlpha)(palette.grey[700], 0.92),
      );
    } else {
      setColor(
        palette.Alert,
      ...
      setColor(
        palette.LinearProgress,
        "infoBg",
        (0, import_colorManipulator.private_safeDarken)(palette.info.main, 0.5),
      );
      setColor(
        palette.LinearProgress,
        "successBg",
  ...
  const { vars: themeVars, generateCssVars } = prepareCssVars_default(
    theme,
    parserConfig,
  );
...
var excludeVariablesFromRoot = (cssVarPrefix) => [
  ...[...Array(24)].map(
    (_, index) =>
      `--${cssVarPrefix ? `${cssVarPrefix}-` : ""}overlays-${index + 1}`,
  ),
...
var { CssVarsProvider, useColorScheme, getInitColorSchemeScript } =
  createCssVarsProvider({
...
//# sourceMappingURL=chunk-NFFIDNQB.js.map
